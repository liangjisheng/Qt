    ...
    :QDialog(parent)
{
    ...
    connect(nameBtn, &QPushButton::clicked, this, ChangeName);
    connect(sexBtn, &QPushButton::clicked, this, ChangeSex);
    connect(ageBtn, &QPushButton::clicked, this, ChangeAge);
    connect(scoreBtn, &QPushButton::clicked, this, ChangeScore);
}

void InputDlg::ChangeName()
{
    bool ok;    // ok指示那个按钮被按下，确定被按下ok为true， 取消被按下，ok为false
...
void InputDlg::ChangeAge()
{
    ...
    int age = QInputDialog::getInt(this, tr("标准int类型输入对话框"),
                                   tr("请输入年龄:"), ageLabel2->text().toInt(&ok),
                                   0, 100, 1, &ok);
    if (ok)
